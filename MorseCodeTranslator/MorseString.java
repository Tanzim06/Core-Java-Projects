...
public class MorseString {
	...
	private static final Pattern VALID_MORSE_PATTERN=
			Pattern.compile("("+Pattern.quote(DASH)
			+"|"+Pattern.quote(DOT)
	...
	public MorseString(String codes) {
		if(!isValidMorse(codes)) {
			throw new IllegalArgumentException(codes+" is not valid Morse Code");
		}
	...
	private String translate(String code) {
		...
		for(String word: words) {
			...
			String text= result.toString().substring(0, result.length()-1);
			return text;
		}
	 ...
	 * e.g. if the object represent "hi"
	 * in Morse it represents ".... .."
	 */
	 ...
	 * e.g. if the object represents ".... .." in Morse,
	 * it represents "hi"
	 */
